...
class Recent_Comments extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="comments widget">';                 
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {                                                            
                            ...
                            $output .=  '<li class="recentcomments"><div class="post-comment">' .sprintf(_x('%1$s on %2$s  %3$s %4$s', 'widgets'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . $posttitle . '</a></div>' ,'<div class="combine"><div class="post-comm-avatar">'.$image .'</div>','<div class="text line-space">'.$com.'</div></div>') . '</li>';                                
			}
 		}
		$output .= $after_widget;
		echo $output;
